...
from app.schemas.ai import (
    ...
    InterviewPracticeReviewRequest,
)
...
async def interview_prep(
    ...
    result = await service.interview_prep(data)

    if result.get("status") == "success":
        return {"code": 200, "message": "success", "data": result}
    else:
        return {"code": 500, "message": result.get("message", "生成失败"), "data": result}


@router.post("/interview-practice-review")
async def interview_practice_review(
    req: InterviewPracticeReviewRequest,
    payload: dict = Depends(get_current_user),
    service: AIService = Depends(get_ai_service)
):
    """
    ...
    """
    data = req.model_dump()
    result = await service.interview_practice_review(data)

    if result.get("status") == "success":
        return {"code": 200, "message": "success", "data": result}
    else:
        return {"code": 500, "message": result.get("message", "点评失败"), "data": result}
