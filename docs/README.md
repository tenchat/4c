...
> 详细清洗规则见: [docs/cleaning-rules.md](./cleaning-rules.md) 数据字典见: [docs/data-dictionary.md](./data-dictionary.md) 导入日志见: [docs/import-log.md](./import-log.md)

#### 招聘数据集（train_data/）

| 文件                  | 规模                | 说明                      |
| --------------------- | ------------------- | ------------------------- |
| `table1_user.csv`     | ~2.6 MB, ~4,500行   | 用户画像数据              |
| `table2_jd_part*.csv` | ~287 MB, ~540,000行 | 职位描述数据（分3个文件） |
| `table3_action.csv`   | ~50 MB, ~700,000行  | 用户行为数据              |

#### 测试数据集（zhaopin_round1_test_20190716/）

| 文件                                 | 规模               | 说明        |
| ------------------------------------ | ------------------ | ----------- |
| `user_ToBePredicted.csv`             | 285 KB, ~2,000行   | 待预测用户  |
| `zhaopin_round1_user_exposure_*.csv` | ~5.2 MB, ~40,000行 | 曝光数据A/B |

#### 就业选择数据集

| 文件                                     | 规模              | 说明            |
| ---------------------------------------- | ----------------- | --------------- |
| `大学生就业选择.csv`                     | 763 KB, ~10,000行 | ⚠️ 存在编码问题 |
| `大学生就业选择(2).csv`                  | 763 KB, ~10,000行 | ⚠️ 存在编码问题 |
| `大学生毕业就业选择样例数据_10000条.csv` | 642 KB, ~10,000行 | ⚠️ 存在编码问题 |

#### 学校数据（学校数据/）

8个Excel文件，包含各学历/学院的就业去向落实率、区域流向等统计数据。

### 6.2 数据质量说明

详见 [docs/data-analysis-report.md](./data-analysis-report.md) Section 8 风险清单。

**已知问题**:

- 大学生就业选择.csv系列文件编码混用GB2312/GBK/UTF-8
- 薪资码字段（desire_jd_salary_id）解码规则不明
- min_years字段存在异常值（如510），疑似编码问题
